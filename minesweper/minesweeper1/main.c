...
//Get informations from the files
int loadlist();
...
//coordinates
COORD coord= {0,0};//windows.h data structure
int main()
{
    playgame();
...
void save(int raws,int colums,int OriginalGrid[raws][colums],int PlayingGrid[raws][colums])//saving game
{
...
int loadlist(char *filename)//loading any file
{
...
void play(int rows,int colums,int OriginalGrid[rows][colums],int PlayingGrid[rows][colums])//playing game
{
...
void win(int rows,int colums, int OriginalGrid[rows][colums],int PlayingGrid[rows][colums])//check win
{
...
void *thread(void *x)//time thread
{
...
void playgame()//game loop
{
    ...
    while(choose != 1 && choose != 2 && choose != 3);
        if(choose == 1)
        {
            ...
            while(!(x>=2 && x <= 30))
            {
                printf("\t\t\t\t\t\tThe width must be between 2 & 30 :");
                x = getint();
            }
            printf("\n\n\n\n\n\n\n\n\n\n\t\t\t\t\t\tEnter the height : ");
            y = getint();
            while(!(y>=2 && y <= 30))
            {
                printf("\t\t\t\t\t\tThe height must be between 2 & 30 :");
                y = getint();
            }
            textcolor(7);
            clear();
...
void savescore(char names[100],double score)//saving score
{
